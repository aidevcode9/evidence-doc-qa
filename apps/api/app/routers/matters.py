...
from app.db import (
    create_matter_with_creator_access,
    get_matter_last_questions,
    get_matter_summary,
    list_documents_for_matter,
    list_matters_for_tenant,
    update_matter_display_name,
...
@router.get("/v1/matters/{matter_id}")
async def get_matter(
    matter_id: str,
    ctx: TenantContext = Depends(get_tenant_context),
) -> dict[str, Any]:
    """Get a single matter summary, including zero-doc matters."""
    if not has_permission(ctx.user_role, "query"):
        raise HTTPException(status_code=403, detail="Permission denied.")
    ...
    matter = get_matter_summary(matter_id=matter_id, tenant_id=ctx.tenant_id)
    if matter is None:
        raise HTTPException(status_code=404, detail="Matter not found.")

    q_data = get_matter_last_questions(
        tenant_id=ctx.tenant_id,
        matter_ids=[matter_id],
    ).get(matter_id, {"last_question_at": None, "last_question_preview": None})
    matter["last_question_at"] = q_data["last_question_at"]
    matter["last_question_preview"] = q_data["last_question_preview"]
    return matter
...
async def create_matter(
    ...
    """
    if not has_permission(ctx.user_role, "create_matter"):
        raise HTTPException(status_code=403, detail="Permission denied.")
    ...
    matter, created = create_matter_with_creator_access(
        body.matter_id,
        ctx.tenant_id,
        display_name,
        creator_user_id=ctx.user_id,
        creator_role=ctx.user_role,
    )

    if not created and not user_has_matter_access(
        user_id=ctx.user_id,
        tenant_id=ctx.tenant_id,
        ...
        raise HTTPException(
            status_code=409,
            detail="Matter already exists and you do not have access to it.",
        )

    if not created and response is not None:
        response.status_code = 200

    return {"matter_id": matter.matter_id, "display_name": matter.display_name}
