...
"""Export router for Q&A session export (FR-032) and cited-only packet (FR-033)."""
...
from app.services.export_service import (
    generate_cited_packet_docx,
    generate_cited_packet_pdf,
...
@router.get("/v1/sessions/{session_id}/export/cited-packet")
async def export_cited_packet(
    session_id: str,
    background_tasks: BackgroundTasks,
    context: RequestContext = Depends(get_request_context),
    format: Literal["pdf", "docx"] = Query("pdf", description="Export format"),
    ...
    """Export cited-only packet listing referenced exhibits/pages (FR-033).

    Generates a focused document listing only the exhibits and pages
    actually cited in the Q&A session.
    """
    # RBAC check (FR-003)
    if not has_permission(context.user_role, "export"):
        raise HTTPException(
    ...
    # Generate cited packet
    if format == "pdf":
        content = generate_cited_packet_pdf(session, messages)
        media_type = "application/pdf"
        suffix = ".pdf"
    else:
        content = generate_cited_packet_docx(session, messages)
        media_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        suffix = ".docx"

    session_short = session_id[:8] if len(session_id) > 8 else session_id
    filename = f"cited-packet-{session_short}{suffix}"
