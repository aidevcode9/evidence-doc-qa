...
export function MessageBubble({ message, onClick, isSelected }: { message: Message; onClick?: () => void; isSelected?: boolean }) {
  ...
  return (
    ...
    >
      <div
        className={`relative max-w-[90%] sm:max-w-[80%] rounded-2xl px-5 py-4 transition-all cursor-default ${isUser
            ? "bg-white text-black"
            : isRefusal
      ...
      >
        ...
        <div className={`text-[10px] font-bold uppercase tracking-widest mb-2 ${isUser ? "text-gray-400" : "text-blue-500"
          }`}>
