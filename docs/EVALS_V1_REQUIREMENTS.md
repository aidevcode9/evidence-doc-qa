...
## 1) Non-negotiables (Director bar)
...
4. **Provenance on every run**  
   - Record `prompt_version`, `verifier_prompt_version`, `model_id`, `retrieval_version`, `parser_mode`, `docs_snapshot` (and reranker id if used).
...
- `docs_snapshot_id` (which docs must be loaded)
- `expected_behavior` (`answer` or `refuse`)
- If `answer`: `expected_doc_id`, `expected_page_num` (optional `expected_keywords`)

**Current coverage (v1, 15 total):**
- Answerable: **6**
- Refusal: **4**
- Adversarial: **3**
- Table/Layout: **2**
...
- >=1 citation matches `expected_doc_id` + `expected_page_num` for answerable cases
...
- Citation coverage >= **0.90** (of answered)
...
prompt_version - verifier_prompt_version - model_id - retrieval_version - parser_mode - docs_snapshot - reranker_id (if used)
